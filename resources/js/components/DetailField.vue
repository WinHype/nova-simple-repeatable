          ...
          <tbody>
            ...
            >
              <td
                ...
                style="height: 2rem"
                v-for="(rowField, j) in row.fields.filter((item, index, self) => self.findIndex(t => t.attribute === item.attribute) === index)"
                :key="j"
              >
                <component
                  :key="j"
...
export default {
  ...
  computed: {
    ...
    headers() {
      ...
      return rows && rows[0] && rows[0].fields.filter((item, index, self) => self.findIndex(t => t.attribute === item.attribute) === index).map(field => ({ name: field.name, attribute: field.attribute }));
    },
  },
