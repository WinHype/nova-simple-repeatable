export default {
  ...
  mounted() {
    // console.log('this.field.rows', this.field);
    this.rows = this.field.rows.map((row, rowIndex) => this.copyFields(row.fields, rowIndex));
    // console.log('rows', this.rows);
    // this.fields = (this.field.fields || []).filter(field => field.component !== 'hidden-field' && field.visible);
  ...
  methods: {
    ...
    copyFields(fields, rowIndex = void 0) {
      ...
      // this.fields = fields.filter(field => field.component !== 'hidden-field' && field.visible);

      // Return an array of fields with unique attribute
      return fields.map(field => {
        let uniqueAttribute = `${this.field.attribute}---${field.attribute}---${rowIndex}---${field.component}`;

        if (!field.dependsOn) {
          uniqueAttribute = `${this.field.attribute}---${field.attribute}---${rowIndex}`;
        }
